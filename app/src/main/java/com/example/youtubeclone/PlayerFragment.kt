...
class PlayerFragment: Fragment(R.layout.fragment_player) {
    ...
    private fun initPlayer(fragmentPlayerBinding: FragmentPlayerBinding){
        ...
        binding?.let {  player?.addListener(object: Player.EventListener{
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                super.onIsPlayingChanged(isPlaying)

                if(isPlaying){
                    it.btnBottomPlayerControl.setImageResource(R.drawable.ic_baseline_pause_24)
                } else {
                    it.btnBottomPlayerControl.setImageResource(R.drawable.ic_baseline_play_arrow_24)
                }
            }
        })}
    ...
    private fun initControlButton(fragmentPlayerBinding: FragmentPlayerBinding){
        fragmentPlayerBinding.btnBottomPlayerControl.setOnClickListener {
                ...
                .enqueue(object: Callback<VideoDto> {
                    override fun onResponse(call: Call<VideoDto>, response: Response<VideoDto>) {
                        if(response.isSuccessful.not()){
                            Log.d("MainActivity", "respponse fail")
                            return
                        }
